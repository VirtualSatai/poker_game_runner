...
class testObservation(unittest.TestCase):
    ...
    def test_init_headsup(self):
        ...
        self.assertEquals(len(obs.player_infos), 2)
    ...
    def test_init_3way(self):
        ...
        self.assertTupleEqual(obs.board_cards, tuple())
        self.assertEquals(len(obs.player_infos), 3)
        assertPlayerInfo(obs.player_infos[0], 5, 1000-5, True)
        assertPlayerInfo(obs.player_infos[1], 10, 1000-10, True)
    ...
    def test_action_str(self):
        ...
        self.assertEquals(action_str, "Player 0: Raise to 300")
        action_str = obs.action_to_str(1)
        self.assertEquals(action_str, "Player 0: Call")
        action_str = obs.action_to_str(0)
        self.assertEquals(action_str, "Player 0: Fold")

        action_str = obs.action_to_str(300, 1)
        self.assertEquals(action_str, "Player 1: Raise to 300")
        action_str = obs.action_to_str(1, 1)
        self.assertEquals(action_str, "Player 1: Call")
        action_str = obs.action_to_str(0, 1)
        self.assertEquals(action_str, "Player 1: Fold")

    def test_methods_headsup(self):
        obs = self.infoStateHeadsup.to_observation(0, [0,1] + [i for i in range(20, 1001-5)])

        self.assertTrue(obs.can_raise())
        self.assertEquals(obs.get_min_raise(), 20)
        self.assertEquals(obs.get_max_raise(), 1000-5)
        self.assertEquals(obs.get_max_spent(), 10)
        ...
        self.assertTrue(obs.can_raise())
        self.assertEquals(obs.get_min_raise(), 90)
        self.assertEquals(obs.get_max_raise(), 1000-5)
        ...
        self.assertEquals(obs.get_min_raise(), 1)
        self.assertEquals(obs.get_max_raise(), 1)
        self.assertEquals(obs.get_max_spent(), 1000)
        self.assertEquals(obs.get_call_size(), 900)
        self.assertEquals(obs.get_pot_size(), 1100)
        self.assertEquals(obs.get_fraction_pot_raise(0.5), 1)
        self.assertEquals(obs.get_fraction_pot_raise(1), 1)
        self.assertEquals(obs.get_fraction_pot_raise(2), 1)
    ...
    def test_methods_3way(self):
        ...
        self.assertEquals(obs.get_max_raise(), 1000-5)
        self.assertEquals(obs.get_max_spent(), 10)
        self.assertEquals(obs.get_call_size(), 10)
        self.assertEquals(obs.get_pot_size(), 15)
        self.assertEquals(obs.get_fraction_pot_raise(0.5), 22)
        self.assertEquals(obs.get_fraction_pot_raise(1), 35)
        self.assertEquals(obs.get_fraction_pot_raise(2), 60)

        self.assertEquals(obs.get_player_count(),3)
        self.assertEquals(len(obs.get_active_players()), 3)
        assertPlayerInfo(obs.get_active_players()[0], 5, 1000-5, True)
        assertPlayerInfo(obs.get_active_players()[1], 10, 1000-10, True)
        ...
        self.assertEquals(obs.get_max_raise(), 1)
        self.assertEquals(obs.get_max_spent(), 1000)
        self.assertEquals(obs.get_call_size(), 950)
        self.assertEquals(obs.get_pot_size(), 1055)
        self.assertEquals(obs.get_fraction_pot_raise(0.5), 1) 
        self.assertEquals(obs.get_fraction_pot_raise(1), 1) 
        self.assertEquals(obs.get_fraction_pot_raise(2), 1) 
    ...
    def test_hand_type(self):
        ...
        self.assertEquals(obs.get_my_hand_type(), HandType.HIGHCARD)
        self.assertEquals(obs.get_board_hand_type(), HandType.HIGHCARD)
        ...
        self.assertEquals(obs.get_my_hand_type(), HandType.THREEOFAKIND)
        self.assertEquals(obs.get_board_hand_type(), HandType.PAIR)

        self.infoState3way.update_info_state_draw(26)

        obs = self.infoState3way.to_observation(2, [0,1] + [i for i in range(20, 1001-5)])

        self.assertEquals(obs.get_my_hand_type(), HandType.FULLHOUSE)
        self.assertEquals(obs.get_board_hand_type(), HandType.PAIR)
